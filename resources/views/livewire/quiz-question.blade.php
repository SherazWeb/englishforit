<div class="quiz-question p-5 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800"
    x-data="{
        selected: null,
        submitted: false,
        correctKey: @js($correctKey),
        explanation: @js($explanation ?? ''),
        counter : 0,
        get isCorrect() { return this.selected === this.correctKey }
    }"
    :class="{
        ...
        <div class="flex-1">
            ...
            <div class="space-y-2">
                @foreach ($options as $key => $option)
                    <label x-data="{ myKey: @js($key) }" class="flex items-center p-3 rounded-lg cursor-pointer transition"
                        :class="{
                        ...
                        <input type="radio" x-model="selected" :value="myKey" :disabled="submitted"
                        ...
                        <!-- Correct -->
                        <span x-show="submitted && myKey === correctKey" class="ml-auto text-green-500">✓</span>
                        <!-- Incorrect -->
            ...
            <button @click="$wire.saveAnswer(selected, isCorrect); submitted = true" :disabled="!selected || submitted"
            ...
            <div x-show="submitted && explanation" x-transition class="mt-3 p-3 rounded-lg"
                :class="{
